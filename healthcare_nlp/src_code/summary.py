...
class TextSummary:
    ...
    """
    def __init__(self, text: str, above_avg_factor: float, language: str="english") -> str:
        self.text = text
    ...
    def create_word_frequency_count(self) -> dict:
        """
    ...
    def _word_in_sentence(self,  sentence: str, sentences_value: dict) -> dict:
        ...
        """
        for word, freq in self.create_word_frequency_count().items():
            if word in sentence:
    ...
    def create_sentence_count(self):
        """
        ...
        for sentence in sentences:
            sentences_value = self._word_in_sentence(sentence=sentence, sentences_value=sentences_value)
        return sentences_value
    ...
    def calculate_average_values(self):
        return int(self.calculate_sum_value() / len(self.create_sentence_count()))

    def summarize(self):
        summary=""
        for sentence in self._get_sentences():
            if (sentence in self.create_sentence_count()) and (self.create_sentence_count()[sentence]>(self.above_avg_factor * self.calculate_average_values())):
                summary += " " + sentence
        return summary